...
export const selectContentVariants = cva('z-9999 min-w-full overflow-y-auto rounded-md border bg-slate-800 text-white border-slate-600 shadow-lg animate-in fade-in-0 zoom-in-95 cursor-pointer pointer-events-auto');
export const selectItemVariants = cva(
  'relative flex min-w-full cursor-pointer text-nowrap items-center gap-2 rounded-sm mb-0.5 py-1.5 pr-8 pl-2 text-sm outline-hidden select-none hover:bg-slate-700 hover:text-white data-selected:bg-amber-500 data-selected:text-white data-disabled:pointer-events-none data-disabled:opacity-50 data-disabled:cursor-not-allowed data-disabled:hover:bg-transparent data-disabled:hover:text-current [&_svg:not([class*="text-"])]:text-white [&_svg]:pointer-events-none [&_svg]:shrink-0 [&_svg:not([class*="size-"])]:size-4 pointer-events-auto',
);
